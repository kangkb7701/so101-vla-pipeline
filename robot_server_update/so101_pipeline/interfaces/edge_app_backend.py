...
def build_app(store: CommandStore, jpeg_source: Callable[[], Optional[bytes]], mjpeg_period_s: float = 0.033) -> FastAPI:
    ...
    def command_voice(cmd: VoiceCommand):
        text = cmd.text.strip()
        # 해석 불가 명령도 기록은 남긴다 (로봇 쪽은 어차피 무시). 앱은 understood로 피드백을 준다.
        task = normalize_command(text)
        entry = store.record_instruction(text)
        return {"ok": True, "understood": task is not None, "task": task, "ts": entry["ts"]}
